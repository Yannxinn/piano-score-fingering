...
class Hand:
    ...
    def __init__(self, noteseq: Sequence[INote], side: str = "right", size: str = "M") -> None:
        ...
        self.cost = -1.0
        self._optimize_cache: dict[tuple, tuple[tuple[int, ...], float]] = {}
        self.cache_hits = 0
        self.cache_misses = 0
    ...
    def ave_velocity(self, fingering: Sequence[int], notes: Sequence[INote]) -> float:
        ...
        # A sustained scale fragment that begins on a white key normally
        # starts from the available outer finger instead of forcing an early
        # crossing. This is a soft preference, not a hard template.
        if self.depth >= 4 and not any(note.isChord for note in notes[:4]):
            opening_deltas = [int(notes[i].pitch) - int(notes[i - 1].pitch) for i in range(1, 4)]
            same_direction = all(delta > 0 for delta in opening_deltas) or all(
                delta < 0 for delta in opening_deltas
            )
            if same_direction and all(0 < abs(delta) <= 2 for delta in opening_deltas):
                target = 1 if (opening_deltas[0] > 0) == (self.LR == "right") else 5
                if not notes[0].isBlack and int(fingering[0]) != target:
                    ergonomic_penalty += 6.0
        ...
        for i in range(1, self.depth):
            ...
            if not na.isChord and not nb.isChord:
                pitch_delta = int(nb.pitch) - int(na.pitch)
                if pitch_delta:
                    natural_sign = 1 if (pitch_delta > 0) == (self.LR == "right") else -1
                    finger_delta = int(fb) - fa
                    stepwise = abs(pitch_delta) <= 2

                    # Stepwise playing normally advances through adjacent
                    # fingers. Skipping a free finger adds needless rotation.
                    if abs(pitch_delta) <= 4 and finger_delta * natural_sign > 0:
                        ergonomic_penalty += max(0, abs(finger_delta) - 1) * 2.0

                    # Prefer a prepared 3->1 scale crossing over repeatedly
                    # tucking the thumb after finger 2 or stretching past 4/5.
                    thumb_under = fb == 1 and fa > 1 and finger_delta * natural_sign < 0
                    finger_over = fa == 1 and fb > 1 and finger_delta * natural_sign < 0
                    if stepwise and thumb_under:
                        ergonomic_penalty += {2: 7.0, 3: 0.15, 4: 0.8, 5: 3.5}[fa]
                    elif stepwise and finger_over:
                        ergonomic_penalty += {2: 1.2, 3: 0.15, 4: 0.7, 5: 3.0}[int(fb)]

                    # Do not strand the hand on an outer finger while a short
                    # scale continues in the same direction. An outer finger
                    # remains natural at a true turning point or phrase end.
                    if i + 1 < self.depth and int(fb) == 5:
                        nc = notes[i + 1]
                        next_delta = int(nc.pitch) - int(nb.pitch)
                        if (
                            not nc.isChord
                            and 0 < abs(next_delta) <= 2
                            and (next_delta > 0) == (pitch_delta > 0)
                        ):
                            ergonomic_penalty += 4.0
            self.set_fingers_positions(
                fingering,
                notes,
                i,
        ...
        return (vmean + ergonomic_penalty) / (self.depth - 1) + chord_penalty
    ...
    def optimize_seq(self, nseq: Sequence[INote], istart: int) -> tuple[list[int], float]:
        ...
        if self.autodepth:
            if nseq[0].isChord:
                ...
                tn0 = nseq[0].time
                for i in range(4, self.max_auto_depth + 1):
                    self.depth = i
        ...
        cache_key = None
        if not self.preserve_posture_memory:
            base_time = float(nseq[0].time)
            chord_ordinals: dict[int, int] = {}
            note_signature = []
            for note in nseq[:depth]:
                chord_id = int(note.chordID) if note.isChord else 0
                if chord_id and chord_id not in chord_ordinals:
                    chord_ordinals[chord_id] = len(chord_ordinals) + 1
                note_signature.append((
                    int(note.pitch), round(float(note.x), 4), round(float(note.time) - base_time, 4),
                    round(float(note.duration), 4), int(bool(note.isBlack)), int(bool(note.isChord)),
                    chord_ordinals.get(chord_id, 0), int(note.chordnr), int(note.NinChord),
                ))
            cache_key = (self.LR, round(self.hf, 4), depth, int(istart), tuple(note_signature))
            cached = self._optimize_cache.get(cache_key)
            if cached is not None:
                self.cache_hits += 1
                fingers, cost = cached
                return list(fingers), cost
            self.cache_misses += 1
        ...
        if cache_key is not None:
            self._optimize_cache[cache_key] = (tuple(best_fingering), minvel)
            ...
            for i in range(n_total):
                ...
                if isinstance(raw_finger, str):
                    ...
                    if text.lstrip("+-").isdigit():
                        raw_finger = int(text)
                if an.is_anchor and isinstance(raw_finger, int):
                ...
                best_finger = 0
                if i > n_total - 10:
                    # Reuse only finger slots that were actually evaluated. With a
                    # manual depth below nine, the remainder of ``out`` is zero-filled;
                    # treating those placeholders as finger numbers used to leave the
                    # tail of short phrases unassigned.
                    if len(out) > 1 and 1 <= int(out[1]) <= 5:
